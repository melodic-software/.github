...
4. Open a pull request, fill out the template, and link the related issue.
   - Title it using [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) with one of the standard types, in the form `<type>[optional scope]: <description>`, for example `feat(api): add pagination`. The title becomes the squash-merge commit subject on the default branch.
